WebGL.prototype.Lights = function(gl, shaders) {
	var prog = shaders.program(),
		lights =
	this.lights = {
		_dir: [],
		uDirAct: [],
		...
		dir: function(light) {
			return this._dir[light] || (
				this.nbDir >= shaders.DIRLIGHTS_MAX
					? null
					: (this._dir[light] = new WebGL.Light(this, gl, this.nbDir++))
			);
		}
	};
...
WebGL.Light = function(lights, gl, id) {
	this.gl = gl;
